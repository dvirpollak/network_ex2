...
# --------------------------------
# return the path of specific ID
...
def sending_updates(list_of_updates, client, identifier):
    while list_of_updates:
        ...
        if cmd == b'UPLOAD':
            time.sleep(1)
            try:
                file_to_send = open(os.path.join(SERVER_DATA_PATH, identifier, data[1].decode(FORMAT)[1:]), "rb")
            except OSError:
                continue
            client.send(full_data)
            client.recv(REC_ACK)
        ...
        while True:
            ...
            if cmd == b"CHECK_FOR_UPDATE":
                ...
                if not DATA_BASE[identifier][computer_id]:
                    client.send(b"NO_UPDATES")
                    break
                sending_updates(DATA_BASE.get(identifier).get(computer_id), client, identifier)
                break
            ...
            # move file from source_path to destination_path
                ...
                try:
                    identifier = data[3].decode(FORMAT)
                    computer_id = data[4].decode(FORMAT)
                except IndexError:
                    pass
